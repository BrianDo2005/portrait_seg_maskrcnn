...
def detection_targets_graph(proposals, gt_class_ids, gt_boxes, gt_masks, config):
    ...
    roi_gt_box_assignment = tf.cond(tf.greater(tf.shape(positive_overlaps)[1], 0), true_fn = lambda: \
        tf.argmax(positive_overlaps, axis=1), false_fn = lambda: tf.cast(tf.constant([]),tf.int64) )
...
def mrcnn_mask_loss_graph(target_masks, target_class_ids, pred_masks, roi_mage):
    ...
    roi_mage_shape = tf.shape(roi_mage)
    roi_mage = K.reshape(roi_mage, (-1, roi_mage_shape[2], roi_mage_shape[3]))
    ...
    roi_mage = tf.gather(roi_mage, positive_ix)

    pred_shape = tf.shape(y_pred)
    roi_mage_shape = tf.shape(roi_mage)
    from Conv2dSobel import Conv2dSobel
    Sobel = Conv2dSobel('sobel')
    Ix = Sobel.SobelX(tf.reshape(roi_mage,(roi_mage_shape[0],roi_mage_shape[1],roi_mage_shape[2],1)))
    Iy = Sobel.SobelY(tf.reshape(roi_mage,(roi_mage_shape[0],roi_mage_shape[1],roi_mage_shape[2],1)))
    a = tf.reshape(y_pred,(pred_shape[0],pred_shape[1],pred_shape[2],1))
    b = tf.reshape(y_pred,(pred_shape[0],pred_shape[1],pred_shape[2],1))
    Mx = Sobel.SobelX(a)
    My = Sobel.SobelY(b)
    image_mag = tf.sqrt(Ix*Ix + Iy*Iy +10e-10)
    Ix = Ix/image_mag
    Iy = Iy/image_mag
    mask_mag = tf.sqrt(Mx*Mx + My*My+10e-10)
    Mx = Mx/mask_mag
    My = My/mask_mag
    L = (1 - (Ix*Mx + Iy*My)*(Ix*Mx + Iy*My))
    Lc = tf.reduce_sum(L*mask_mag)/(tf.reduce_sum(mask_mag)+10e-5)
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            _,_,_,roi_image =\
                DetectionTargetLayer(config, name="proposal_image")([
                    target_rois, input_gt_class_ids, gt_boxes, input_image])
            # Network Heads
            ...
            mask_loss = KL.Lambda(lambda x: mrcnn_mask_loss_graph(*x), name="mrcnn_mask_loss")(
                [target_mask, target_class_ids, mrcnn_mask, roi_image])
